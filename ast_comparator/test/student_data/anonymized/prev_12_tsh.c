...
void eval(char *cmdline)
{
  ...
  /* pass empty line */
  if(argv[0] == NULL) return;/* pass empty line */
  ...
  if(!builtin_cmd(argv)){
    ...
    if((pid = fork()) < 0)
      ...
      /* unblock SIGCHLD */
      if(sigprocmask(SIG_UNBLOCK, &set_SIGCHLD, NULL) < 0)
        unix_error("Sigaddset failed");
      ...
      /* add to joblist */
      if(addjob(jobs, pid, is_bg ? BG : FG, cmdline) == 0)
      ...
      if(is_bg){
        /* background case */
        /* just alert and keep going */
        printf("[%d] (%d) %s", pid2jid(pid), pid, cmdline);
      }
      else{
        /* foreground case */ 
        /* wait until it ends or goes bg, etc. */
        waitfg(pid);
      }
    }
  }
...
void sigchld_handler(int sig)
{
  ...
  do{
    ...
    if(pid > 0){
      if(WIFEXITED(status)){
        /* if child is dead, delete from joblist */
        deletejob(jobs, pid);
      }
