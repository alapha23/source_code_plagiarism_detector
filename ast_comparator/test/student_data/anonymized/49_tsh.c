...
void eval(char *cmdline)
{
  ...
  /* Parent : add job */
  if (bg)
    addjob(jobs, c, BG, cmdline);
  else
    addjob(jobs, c, FG, cmdline);
  ...
  if (!bg) {
    ...
    struct job_t *job = getjobpid(jobs, c);
...
void do_bgfg(char **argv)
{
  ...
  job_i = (*argv[1] == '%')? atoi(argv[1] + 1) : atoi(argv[1]);
  if (!job_i) {
  ...
  /* Get job with index */
  job = (*argv[1] == '%')? getjobjid(jobs, job_i) : getjobpid(jobs, job_i);
  if (!job) {
    if (*argv[1] == '%')
