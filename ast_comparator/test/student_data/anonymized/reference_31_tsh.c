...
#define ST 3    /* stopped */
#define BUILTIN_COMMANDS 4 /* how many built-in commands there are */
/*
...
* 重定向，1表示stdout，2表示是stderr 。第一个参数为重定向的目标。
*/
dup2(1, 2);
...
/* block SIGCHLD signals to so it doesn't cry if child terminates
before addjob gets called */
mask_signal(SIG_BLOCK, SIGCHLD);
if((pid = Fork()) == 0) {   /* child runs user job */
mask_signal(SIG_UNBLOCK, SIGCHLD); /* unblock SIGCHLD */
...
if (!bg) waitfg(pid); /* wait for fg job to terminate */
else { /* don't wait for bg jobs */
...
* the user has requested a FG job.  Return -1 on a blank line.
*/
int parseline(const char *cmdline, char **argv) {
...
int builtin_cmd(char **argv) {
char *command = argv[0];
/* see if this is a built in command */
...
*/
void do_bgfg(char **argv) {
/* get the pid (if given a jid) */
struct job_t *job;
char *command = argv[0];
char *id = argv[1]; /* might be pid or jid */
int jid;
/* handle non-existent id error */
if(id == NULL) {
printf("%s command requires PID or %%jobid argument/n", command);
return;
}
/* make sure jid is actually a jid (this assumes jid/pid can't be 0) */
if(id[0] == '%' && (atoi(id += sizeof(char)) != 0)) { /* if this looks like a jid (starts with %) and isn't doesn't look like an error */
jid = atoi(id);
/* try and get the job, print msg on error */
if((job = getjobjid(jobs, jid)) == NULL) {
printf("%%%s: No such job/n", id);
return;
}
} else if(atoi(id) != 0 ) { /* assume this is a PID */
jid = pid2jid(atoi(id));
/* try and get the job, print msg on error */
if((job = getjobjid(jobs, jid)) == NULL) {
printf("(%s): No such process/n", id);
return;
}
} else { /* handle errors */
printf("%s: argument must be a PID or %%jobid/n", command);
return;
}
/* continue the job */
send_signal(-(job->pid), SIGCONT);
/* handle bg/fg */
if(!strcmp("bg", command)) {
printf("[%d] (%d) %s", job->jid, job->pid, job->cmdline);
job->state = BG;
} else if(!strcmp("fg", command)) {
job->state = FG;
waitfg(job->pid); /* wait for the fg job to finish */
} else printf("bg/fg error: %s/n", command);
}
/*
...
/* sleeping for any amount of time caused strange effects when
signals occured during the sleep time */
sleep(0);
}
}
/*****************
...
/* reap any zombies and handle status reports -- the loop allows
reaping of any processes that die during this function */
while((pid = waitpid(-1, &status, WUNTRACED|WNOHANG)) > 0) {
if(WIFSTOPPED(status)) {
/* handle stop signals (this assumes all stop signals are
equivalent to SIGTSTP) */
sigtstp_handler(WSTOPSIG(status));
} else if(WIFSIGNALED(status)) { /* uncaught signal */
child_sig = WTERMSIG(status);
if(child_sig == SIGINT) sigint_handler(child_sig);
else unix_error("sigchld_handler: uncaught signal/n");
} else deletejob(jobs, pid); /* remove the job under normal conditions */
}
...
if(verbose) {
printf("Added job [%d] %d %s/n", jobs[i].jid, jobs[i].pid, jobs[i].cmdline);
}
return 1;
}
}
