...
void eval(char *cmdline)
{
	...
	if (!builtin_cmd(argv)) {
		...
		if ((pid = Fork()) == 0) {
			setpgid(0, 0);     /* set group pid of child as its pid */
...
int builtin_cmd(char **argv)
{
	if (!strcmp(argv[0], "quit"))  			/* quit command */
		...
		if (args[0] == '%' && isdigit(args[1])) { /* deal with jid format */
			jid = atoi(&args[1]);
			pid = getjobjid(jobs, jid)->pid;

			kill(-pid, SIGINT);
		} else if (isdigit(args[0])) { 					  /* deal with pid format */
			pid = atoi(args);

			kill(-pid, SIGINT);
		} else {
			printf("Weird kill argument format\n");
		}
		return 1;
	}
...
void do_bgfg(char **argv)
{
	...
	if (args != NULL) {
		...
		} else {  /* non-valid argument */
			printf("%s: argument must be a PID or %%jobid\n", argv[0]);
			return;
		}
	} else {    /* bg and fg needs argument */
	...
	if (!strcmp(argv[0], "bg")) { /* case bg */
		if (job->state == ST) {     /* only take care for stopped job */
			printf("[%d] (%d) %s", job->jid, job->pid, job->cmdline);
			job->state = BG;
			kill(-pid, SIGCONT);
		}
	} else if (!strcmp(argv[0], "fg")) { /* case fg */
		if (job->state == BG || job->state == ST) { /* only if running in bg or stopped */
			job->state = FG;
			kill(-pid, SIGCONT);
			waitfg(job->pid);
		}
