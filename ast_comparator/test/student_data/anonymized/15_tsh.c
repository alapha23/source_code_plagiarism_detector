...
void eval(char *cmdline)
{
	...
	sigset_t set; //to control signal insie my shell.
	sigemptyset(&set);
	...
	if(!builtin_cmd(argv)){
		...
		if((pid = fork()) == 0){
			...
			setpgid(0,0);
			//create child process and execute the program in the argv[0]
			if(execv(argv[0], argv) < 0){
				//when executing wrong command
		...
		//
			...
			//멈춰놓아야함 sigstop을 보내면될듯 get pid해서
			if(!bg) addjob(jobs, pid, FG, cmdline);
			else addjob(jobs, pid, BG, cmdline);
			sigprocmask(SIG_UNBLOCK, &set, NULL);
			if(!bg){
			//wait for a foreground process to complete
				waitfg(pid);
			}
			else{
				//backgroundjob and print (like reference tsh)
...
int builtin_cmd(char **argv)
{
	char* nump;
	int jid;
	//first, try to execute 'quit' command
	if(strcmp("quit", argv[0]) == 0){
		...
		//find the process id and kill, delete it from the jobs list
		nump = strstr(argv[1], "%");
		nump++;
		jid = atoi(nump);
...
void do_bgfg(char **argv)
{
	...
	if((p = strstr(argv[1], "%")) != NULL){
		...
		p++;
		jobid = atoi(p);
		if(getjobjid(jobs,jobid) != NULL) jobid = getjobjid(jobs,jobid)->pid;
		else{
		...
		jobid = atoi(argv[1]); //as argv[1] is pid
...
void waitfg(pid_t pid)
{
	//if foreground, sleep for a second
...
void sigint_handler(int sig)
{
	...
	if(job!= NULL)	kill(-fgpid(jobs), SIGINT);
  return;
}
...
void sigtstp_handler(int sig)
{
	//exit the process in foreground
	struct job_t *job = getjobpid(jobs, fgpid(jobs));
	if(job!= NULL)	kill(-fgpid(jobs), SIGTSTP);
  return;
}
