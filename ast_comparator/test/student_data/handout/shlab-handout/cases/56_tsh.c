...
if(!!builtin_cmd(tok)) // builtin cmd 
return ; 
pid_t pid; 
sigset_t maskchld,maskint,maskstp; //block three signals 
sigemptyset(&maskchld); //call block function
sigemptyset(&maskint); 
sigemptyset(&maskstp); 
...
if((pid=fork())==0) 
{ 
if(setpgid(0,0)<0) //sperate child and tsh 
unix_error("eval: setpgid failed\n"); 
sigprocmask(SIG_UNBLOCK,&maskchld,NULL); //unblock child 
sigprocmask(SIG_UNBLOCK,&maskint,NULL); 
sigprocmask(SIG_UNBLOCK,&maskstp,NULL); 
signal(SIGINT,SIG_DFL); //recover
signal(SIGTSTP,SIG_DFL); 
signal(SIGTTIN,SIG_DFL); 
signal(SIGTTOU,SIG_DFL); 
signal(SIGCHLD,SIG_DFL); 
if(tok.infile) //child I/O redirection 
{ 
...
if(execve(tok.argv[0],tok.argv,environ)<0) //load new process 
{ 
...
addjob(job_list,pid,BG,cmdline); //add new fork job to joblist
struct job_t *job=getjobpid(job_list,pid); 
printf("[%d] (%d) %s\n",job->jid,job->pid,job->cmdline); 
sigprocmask(SIG_UNBLOCK,&maskchld,NULL);//unblock tsh
sigprocmask(SIG_UNBLOCK,&maskint,NULL); 
sigprocmask(SIG_UNBLOCK,&maskstp,NULL); 
//return ; 
} 
...
if((pid=fork())==0) //child 
{ 
if(setpgid(0,0)<0) //convert child to new process group 
unix_error("eval: setpgid failed\n"); 
...
while((job=getjobpid(job_list,pid))!=NULL&&(job->state==FG)) //wait foreground child to end and turn it to background 
{ 
sleep(0); 
sleep(0); 
sleep(0); 
...
{ 
if(tok.outfile) //job I/O redirection
{ 
int fd1=open(tok.outfile,O_WRONLY,0); 
listjobs(job_list,fd1); // output job list to stdout 
} 
else 
listjobs(job_list,1); // output job list to stdout
return 1; 
} 
...
job->state=BG; //change state of job to BG 
if(kill(-pid,SIGCONT)<0) // sent signal to job 
fprintf(stdout,"kill error\n"); 
printf("[%d] (%d) %s\n",job->jid,job->pid,job->cmdline); 
return 1; 
} 
...
job->state=FG; //change state of job to FG 
if(kill(-pid,SIGCONT)<0) // sent signal to job 
fprintf(stdout,"kill error\n"); 
while((job=getjobpid(job_list,pid))!=NULL&&(job->state==FG)) //wait forehand job 
{ 
sleep(0); 
sleep(0); 
sleep(0); 
...
do 
{ 
pid=waitpid(-1,&status,WNOHANG|WUNTRACED); //deal with child signal
if(pid>0) 
{ 
if(WIFEXITED(status)) //if quit normally delete the state of child
{ 
deletejob(job_list,pid); 
} 
else if(WIFSIGNALED(status)) //stop child
{ 
