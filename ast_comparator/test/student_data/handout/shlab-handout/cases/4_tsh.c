...
static char * builtins [] = { "quit", "jobs", "bg", "fg" };
...
void eval(char *cmdline)
{
	...
	if ((is_built_in(*argv)) != -1) { // execute immediately if built-in
		builtin_cmd(argv);
...
char **cmd_array(char *cmdline, int *length)
{
	char ** res  = NULL;
...
int builtin_cmd(char **argv)
{
	int cmd_index = is_built_in(*argv);
