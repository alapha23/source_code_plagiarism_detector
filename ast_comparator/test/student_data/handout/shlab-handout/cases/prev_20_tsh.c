...
int main(int argc, char **argv)
{
    ...
    Signal(SIGQUIT, sigquit_handler);  /* provides a clean way to kill the shell */
...
int parseline(const char *cmdline, char **argv)
{
    static char arr[MAXLINE];   /* holds local copy of command line */
    char *arr0 = arr;            /* ptr that traverses command line */
    char *sp;                   /* points to first space spiter */
    int argc;                   /* number of args */
    int bg;                     /* background job? */
    
    strcpy(arr0, cmdline);
    arr0[strlen(arr0)-1] = ' ';  /* replace trailing '\n' with space */
    while (*arr0 && (*arr0 == ' ')) /* ignore leading spaces */
        arr0++;
...
int builtin_cmd(char **argv)
{
    /* check if this is a built in command, ignores command if & is the first char */
...
void do_bgfg(char **argv)
{
    ...
    /* If id does not exist - print message */
    if(id == NULL) {
    ...
    /* print error message if it's not both of PID and JID */
    else {
