...
void eval(char *cmdline)
{
	...
	char *argv[MAXARGS];
	argv[0] = NULL;
	int is_bgjob = parseline (cmdline, argv);	
...
void do_bgfg(char **argv)
{
	...
	/* invalid character */
	else if ((*argv[1] != '%') && ((*argv[1] < '0') || (*argv[1] > '9'))) {
...
void sigchld_handler(int sig)
{
	...
	while ((pid = waitpid (-1, &status, WUNTRACED | WNOHANG)) > 0) {
		...
		/* stopped */
			...
			struct job_t *stop_job = getjobpid (jobs, pid);
			stop_job->state = ST;
