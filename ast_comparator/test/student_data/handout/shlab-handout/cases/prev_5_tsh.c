...
void eval(char *cmdline)
{
	...
	bg = parseline(cmdline, argv);	//parse the cmdline, and determine whether it is built_in or not
	if(!builtin_cmd(argv)) { 
...
// see if this is a built in command, ignores command if & is the first char
	...
	else if (!strcmp("jobs", argv[0])) { 	//print the lists of jobs
		listjobs(jobs);  
		return 1;  
	}  
	else if (!strcmp("bg", argv[0]) || !(strcmp("fg", argv[0]))) {  
		do_bgfg(argv);  	//background/foreground
		return 1;  
	}
...
// The id may be a PID or a JID
	struct job_t *job;
	char *id = argv[1];		//id is 2nd part of parsed cmdline
	int jid;	
